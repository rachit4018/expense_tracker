...
const Group = () => {
  ...
  useEffect(() => {
    const fetchGroupDetails = async () => {
      try {
        const response = await axiosInstance.get(`/api/v1/groups/${parsedGroupId}/`, {
          headers: {
            "X-Username": username,
          },
        });
  ...
  const handleAddMember = async () => {
    ...
    try {
      const response = await axiosInstance.post(
        `/api/v1/groups/${parsedGroupId}/add_member/`,
      ...
      if (response.data?.success) {
        ...
        setGroupDetails((prev) => ({
          ...prev,
          members: [...(prev?.members || []), { username: selectedMember }],
        }));

        setAvailableMembers((prev) =>
          prev.filter((member) => member.username !== selectedMember)
        );
  ...
  if (loading) {
    return <p className="text-center mt-10 text-lg text-gray-600">Loading group details...</p>;
  }
      ...
      <div className="py-10 px-4">
        <div className="max-w-5xl mx-auto space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {expenses.map((expense) => (
                  ...
                  >
                    ...
                    <p>
                      <strong>By:</strong> {expense.created_by}
                    </p>
                    <p>
          ...
          <div className="flex flex-col sm:flex-row justify-between gap-4 w-full">
            <button
              onClick={() => navigate(`/expense/${parsedGroupId}`, { state: { user } })}
